...
class DecisionTreeRegressionSurrogate():
...
class DecisionTreeClassificationSurrogate():
    def __init__(self, tree_depth=4):
        self.dr = DecisionTreeClassifier(max_depth=tree_depth)
    ...
    def train(self, est, X_train, y_train):
        ...
        y_pred_est0 = est.predict(folds[0]['X_test'])
        score = roc_auc_score(folds[0]['y_test'],y_pred_est0)
        print("Base estimator score on first fold: {}".format(score))
        ...
        score = roc_auc_score(folds[1]['y_test'],y_pred_dr)
        print("Score for decision tree: {}".format(score))
